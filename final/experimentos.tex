...
Para criação da segmentação manual de referência (\textit{ground-truth}), \textbf{X} voluntários, todos alunos de pós-graduação em informática, foram convidados a realizar a segmentação manual das imagens da base de dados, através de uma ferramenta criada com esta finalidade \textbf{(colocar url?)}. Todos os voluntários foram orientados a segmentar as imagens utilizando critérios definidos nas instruções da própria ferramenta. O conteúdo integral das instruções pode ser encontrado no apêndice \ref{apendice:instrucoesManualSeg}.
