...
\todo[inline]{Lugar certo? Talvez deva estar junto às métricas de avaliação de machine learning, na seção de fundamentação teórica, subseção de avaliação}
...
Com um conjunto de dados altamente desbalanceado, algumas precauções na avaliação de desempenho dos algoritmos precisam ser tomadas. A avaliação do aprendizado precisa considerar os cálculos de precisão e revocação das classes menos representadas no conjunto de dados.
...
A avaliação dos resultados de revocação, bem como a análise da curva ROC da classe de elementos antrópicos para cada algoritmo testado podem ser vistas na tabela \ref{tab:experimentoClassificacao1Revocacao}.
...
\centering
	\begin{tabulary}{\linewidth}{|L|R|R|R|R|}
		\hline
		\textbf{Método} & \textbf{Revocação} & \textbf{Precisão} & \textbf{F1} & \textbf{Área ROC} \\ \hline
		Random Forest           & 0.500 & \cellcolor{gray!25}0.917 & 0.647 & \cellcolor{gray!25}0.999 \\ \hline
		Naive Bayes             & \cellcolor{gray!25}0.955 & 0.054 & 0.101 & 0.984 \\ \hline
		Naive Bayes (CFS)       & 0.909 & 0.030 & 0.057 & 0.978 \\ \hline
		Random Forest (CFS)     & 0.500 & 0.846 & 0.629 & 0.926 \\ \hline
		KNN                     & 0.727 & 0.889 & \cellcolor{gray!25}0.800 & 0.907 \\ \hline
		Árvore de decisão       & 0.636 & 0.700 & 0.667 & 0.839 \\ \hline
		KNN (CFS)               & 0.591 & 0.722 & 0.650 & 0.806 \\ \hline
		SVM (CFS)               & 0.591 & 0.813 & 0.684 & 0.795 \\ \hline
		Árvore de decisão (CFS) & 0.455 & 0.588 & 0.513 & 0.724 \\ \hline
		SVM                     & 0.000 & 0.000 & 0.000 & 0.500 \\ \hline
	\end{tabulary}
\caption{Comparação de métodos de classificação em relação à classe de elementos antrópicos, ordenados pela área ROC}
\label{tab:experimentoClassificacao1Revocacao}
\end{table}

Tanto no aprendizado geral quanto especificamente no aprendizado da classe de elementos antrópicos, o algoritmo \textit{Random Forest} obteve um desempenho médio promissor. É seguro afirmar, também, que a seleção de atributos utilizando o método CFS não teve impacto positivo na generalização e precisão do aprendizado, já que em virtualmente todos os métodos obtiveram melhor desempenho com o conjunto completo de atributos.
