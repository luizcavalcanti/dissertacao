...
Adicionalmente, experimentos foram realizados com técnicas de classificação. O objetivo era saber se poderíamos utilizar apenas uma etapa para realizar segmentação e a classificação dos segmentos. 

\todo[inline]{falar de como foi feito o experimento}
\todo[inline]{falar por que o método foi descartado (muito ruido, complexidade em fechar regiões para calcular o erro de segmentação, etc (como comparar esta etapa com duas etapas diferentes? Talvez fazendo LCE e GCE para as regiões conectadas e precisão/revocação para a classificação dos pixels}

Todos os algoritmos obtiveram precisão inferior ao que foi conseguido na etapa de segmentação isoladamente. Os resultados foram publicados no \textit{$10^{th}$ International Conference on Computer Vision Theory and Applications} e o artigo de \citeonline{cavalcanti:2015} completo pode ser visto no apêndice \ref{cap:visapp2015}.

Para fins de comparação, os resultados deste segundo experimento são apresentados na tabela \ref{tab:experimentoArtigo}, com o melhor resultado para cada critério destacado em coloração cinza. O tempo de execução da classificação para cada imagem no artigo publicado ignora o tempo de extração de características da imagem. Para uma comparação correta com os métodos de segmentação experimentados anteriormente, esse tempo gasto em extração de características foi acrescido na tabela desta seção.
...
A causa para estes resultados reside em dois fatores: a desigualdade na distribuição das classes do conjunto de dados e a baixa tolerância dos métodos convencionais de aprendizagem supervisionada à bases de dados fortemente desbalanceadas. Os métodos utilizaram os mesmos parâmetros do experimento anterior, de classificadores multi-classe.
...
\todo[inline]{Discutir os resultados método a método}
\todo[inline]{Porque 100\% de precisão no KNN para a classe é ruim? entendo que isso pode ser ruim para o aprendizado geral, mas a classe é tão minoritária que não consigo ver como um problema (não achei literatura apoiando nenhum dos dois vereditos)}

Neste experimento, a seleção de atributos através do método CFS obteve bons resultados, reduzindo a dimensionalidade e melhorando a revocação e precisão em quase todos os casos, com exceção do algoritmo \textit{Random Forest}. Os melhores métodos deste experimento não obtiveram desempenho significativamente inferior ao experimento de classificadores multi-classe. Ainda assim, o uso de métodos de aprendizado que não foram concebidos para lidar com uma base de dados com distribuição de classes tão desigual não é o cenário ideal. Por este motivo, experimentos utilizando algoritmos que levem em consideração a detecção de classes extremamente minoritárias devem ser realizados.
...
\todo[inline]{Discutir os resultados método a método}
\todo[inline]{É correto afirmar que apenas OC-SVM aprendeu? Tentei parametrizar o REPTree exaustivamente, sem resultados melhores que este. O que fazer neste caso? }
...
\todo[inline]{falar sobre seleção de atributos}
\todo[inline]{falar sobre parametrização dos métodos}
...
\todo[inline]{Discutir os resultados método a método}
\todo[inline]{fazer link com próximo capítulo}
